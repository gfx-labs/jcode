...
impl JevClient {
    ...
    pub fn for_browser() -> Result<Self> {
        if std::env::var_os(BROWSER_PROVIDER_ENV).is_some() {
            ...
            let config = &crate::config::config().agents.jev;
            let settings = resolve_with_timeout(config, Duration::from_secs(25))?;
            Self::for_browser_settings(settings, &config.provider)
        }
    }
    ...
    fn configured(settings: ResolvedJev, provider: &str, purpose: JevPurpose) -> Result<Self> {
        ...
        Ok(Self {
            ...
            me_endpoint: None,
            model_override: Some(settings.model),
    ...
    pub async fn evaluate(&self, state: Value, questions: Map<String, Value>) -> Result<Value> {
        let body = request_body_for_model(
            self.purpose,
            self.provider,
            self.model_id(),
            state,
...
fn request_body_for_model(
    ...
    questions: &Map<String, Value>,
        ...
        provider != JevProvider::Jcode
            || matches!(
                purpose,
                JevPurpose::Memory | JevPurpose::Browser | JevPurpose::Voice
            ),
...
mod config_resolution_tests {
    ...
    fn browser_rejects_removed_provider_until_explicit_browser_override() {
        ...
        assert_eq!(client.provider_name(), "typesafe");
        ...
        assert!(JevClient::for_browser().is_err());
        crate::env::remove_var(BROWSER_PROVIDER_ENV);
        crate::env::set_var("JCODE_JEV_PROVIDER", "typo");
        crate::config::Config::invalidate_cache();
        assert!(JevClient::for_browser().is_err());
    }
