...
#[test]
fn legacy_skill_suggestion_settings_still_parse_and_are_dropped() {
    // The Jev skill router was removed. Configs written for it must still load,
    // and the obsolete keys must not survive a save round trip.
    let toml_src = "[agents]\n\
...
fn test_generated_default_config_has_expected_user_defaults() {
    ...
    assert!(
        content.contains("memory_jev_provider = \"auto\"")
            && content.contains("memory_jev_threshold = 0.8")
            && content.contains("Recall uses Jev typed Decisions directly"),
    ...
    assert!(
        ...
        "generated default config should document the separate optional Luna extraction sidecar"
    );
    ...
    assert_eq!(parsed.agents.memory_jev_provider, "auto");
    assert_eq!(parsed.agents.memory_jev_threshold, 0.8);
    assert!(parsed.agents.memory_sidecar_enabled);
    assert!(
