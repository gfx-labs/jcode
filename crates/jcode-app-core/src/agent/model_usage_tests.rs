...
#[tokio::test]
async fn mobile_turn_usage_tracks_the_persisted_message_identity() {
    let _home = crate::auth::test_sandbox::AuthTestSandbox::new().unwrap();
    let mut agent = usage_agent(false).await;
    let mobile_id = *uuid::Uuid::new_v4().as_bytes();
    let expected_id = format!("mobile:{}", uuid::Uuid::from_bytes(mobile_id));
    let (tx, _rx) = tokio::sync::mpsc::unbounded_channel();
    agent
        .run_once_streaming_mpsc_with_mobile_id("mobile input", vec![], None, tx, mobile_id)
        .await
        .unwrap();
    let stored = agent
        .session
    ...
    let anchor = agent.session.model_usage_turn_id.as_ref().unwrap();
    assert!(anchor.ends_with(&expected_id));
    let saved = crate::session::Session::load(&agent.session.id).unwrap();
    assert_eq!(saved.model_usage_turn_id.as_ref(), Some(anchor));
    for history in [
        agent.get_history(),
        ...
        let mobile = history
            .iter()
            .find(|m| m.message_id.as_ref() == Some(&expected_id))
            .unwrap();
        assert_eq!(mobile.content, "mobile input");
        assert!(mobile.timestamp_unix_ms.is_some());
    }
    let serving = agent
        .model_routes()
        .into_iter()
        .find(|r| r.model == "serving-model")
        .unwrap();
    assert_eq!(serving.usage.unwrap().count, 1);
}
