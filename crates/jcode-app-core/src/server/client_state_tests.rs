...
async fn handle_get_history_falls_back_to_persisted_snapshot_when_agent_is_busy() {
    for tier in [Some("priority"), Some("flex"), None] {
        for timestamp in [None, Some(1_789_096_075_695_i64)] {
            assert_history_service_tier_and_pdf_capability(tier, true, false, false, timestamp)
                .await;
        }
    }
}
...
async fn pdf_panels_history_capability_covers_live_and_persisted_paths() {
    for busy in [false, true] {
        for supports_pdf_panels in [false, true] {
            assert_history_service_tier_and_pdf_capability(
                None,
                busy,
                supports_pdf_panels,
                false,
...
async fn handle_get_history_uses_live_snapshot_when_agent_is_available() {
    assert_history_service_tier_and_pdf_capability(None, false, false, false, None).await;
}

#[tokio::test]
async fn history_guard_survives_racing_turn_and_is_released_before_write() {
    assert_history_service_tier_and_pdf_capability(None, false, false, true, None).await;
}
