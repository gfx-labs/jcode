...
pub(in crate::tui::app) async fn submit_prepared_remote_input(
    ...
    if app.remote_model_switch_in_flight
        ...
        || app.agent_profile_switch_pending()
    {
        app.pending_prompt_after_model_switch = Some(prepared);
        app.set_status_notice(if app.agent_profile_switch_pending() {
            "Prompt queued until agent profile change completes"
        } else if app.auth_catalog_refresh_pending {
